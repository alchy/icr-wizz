...
export const VelocityEditor: React.FC = () => {
  ...
  useEffect(() => {
    ...
    gridX.selectAll('line').attr('stroke', '#3E4044').attr('stroke-width', 0.5)
    gridX.select('.domain').remove()

    const gridY = g.append('g').attr('class', 'grid')
      .call(d3.axisLeft(yScale).ticks(5).tickSize(-IW).tickFormat(() => ''))
    gridY.selectAll('line').attr('stroke', '#3E4044').attr('stroke-width', 0.5)
